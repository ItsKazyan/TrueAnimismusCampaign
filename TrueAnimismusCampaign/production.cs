...
public static class ProductionManager
{
    ...
    public static Texture fetchTexture(string filePath)
    {
        ...
        {
            Logger.Log("[TrueAnimismusCampaign] fetchProductionTexture: Couldn't load '" + filePath + ".png', will use the generic vial texture instead.");
        }
        productionTextureBank.Add(filePath, ret);
        return ret;
    }

    public static void AddOverlaysForPuzzle(string puzzleID, List<CabinetModelTAC.OverlayModelTAC> overlays)
    {
        ...
        foreach (var overlay in overlays)
        {
            ...
            {
                var position = ModelHelpersTAC.Vector2FromString(overlay.Position);
